;;; sr-speedbar-autoloads.el --- automatically extracted autoloads
;;
;;; Code:


;;;### (autoloads (sr-speedbar-refresh-toggle sr-speedbar-refresh-turn-off
;;;;;;  sr-speedbar-refresh-turn-on sr-speedbar-select-window sr-speedbar-close
;;;;;;  sr-speedbar-open sr-speedbar-toggle) "sr-speedbar" "sr-speedbar.el"
;;;;;;  (19851 31309))
;;; Generated autoloads from sr-speedbar.el

(autoload 'sr-speedbar-toggle "sr-speedbar" "\
Toggle sr-speedbar window.
Toggle visibility of sr-speedbar by resizing
the `sr-speedbar-window' to a minimal width
or the last width when visible.
Use this function to create or toggle visibility
of a speedbar-window.  It will be created if necessary.

\(fn)" t nil)

(autoload 'sr-speedbar-open "sr-speedbar" "\
Create `sr-speedbar' window.

\(fn)" t nil)

(autoload 'sr-speedbar-close "sr-speedbar" "\
Close `sr-speedbar' window and save window width.

\(fn)" t nil)

(autoload 'sr-speedbar-select-window "sr-speedbar" "\
Force the windows that contain `sr-speedbar'.

\(fn)" t nil)

(autoload 'sr-speedbar-refresh-turn-on "sr-speedbar" "\
Turn on refresh content automatically.

\(fn)" t nil)

(autoload 'sr-speedbar-refresh-turn-off "sr-speedbar" "\
Turn off refresh content automatically.

\(fn)" t nil)

(autoload 'sr-speedbar-refresh-toggle "sr-speedbar" "\
Toggle refresh content status.

\(fn)" t nil)

;;;***

;;;### (autoloads nil nil ("sr-speedbar-pkg.el") (19851 31310 89950))

;;;***

(provide 'sr-speedbar-autoloads)
;; Local Variables:
;; version-control: never
;; no-byte-compile: t
;; no-update-autoloads: t
;; coding: utf-8
;; End:
;;; sr-speedbar-autoloads.el ends here
